...
export class ResonanceModel implements TModel {
  ...
  public step(dt: number): void {
    ...
    const frequency = this.circuit.source.frequencyProperty.value;
    ...
    this.circuit.source.frequencyProperty.value = Math.min(
      range.max,
      Math.max(range.min, range.min * 10 ** (next * decades)),
    );
  }
