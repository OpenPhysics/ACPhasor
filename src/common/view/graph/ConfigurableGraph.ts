 ...
 *
 * Ported from Resonance. The sub-step (RK4 high-resolution data) path was removed because
 * ACPhasor's models are single-step — the graph samples each axis property's current value
 * once per frame via {@link addDataPoint}. Colors and namespace were remapped to ACPhasor,
...
export default class ConfigurableGraph extends Node {
  ...
  public constructor(
    ...
    this.chartRectangle = new ChartRectangle(this.chartTransform, {
      fill: ACPhasorColors.graphBackgroundProperty,
    ...
    this.verticalGridLineSet = new GridLineSet(this.chartTransform, Orientation.VERTICAL, initialSpacing, {
      stroke: ACPhasorColors.gridLinesProperty,
    ...
    this.linePlot = new LinePlot(this.chartTransform, [], {
      stroke: ACPhasorColors.plotColorProperty,
...
ACPhasorNamespace.register("ConfigurableGraph", ConfigurableGraph);
