...
```
main.ts
  └─ ACPhasorScreen            (Screen<ACPhasorModel, ACPhasorScreenView>)
       ├─ ACPhasorModel         owns all state
       └─ ACPhasorScreenView    owns all visuals
```
...
existing `ACPhasorScreen.ts`. Screen icons live in one shared module under
